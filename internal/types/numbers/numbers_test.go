...
func Test_Numbers(t *testing.T) {
	...
	t.Run("Test floating point math to see if its correct with shopspring/decimal", func(t *testing.T) {
		...
		fmt.Printf("Expected: %+v\nActual  : %+v\n", expectedRawTokensPerDay, rawTokensPerDay.BigInt().String())
		...
		// round := one.Div(two)
		// fmt.Printf("Pre multiplication: %+v\n", rawTokensPerDayForRounding.String())
		//
		// f := rawTokensPerDayForRounding.BigFloat()
		// fRounded := f.Mul(f, round.BigFloat())
		// fmt.Printf("Rounded: %+v\n", fRounded)
		// fRoundedInt := new(big.Int)
		//
		// fRoundedInt, _ = fRounded.Int(fRoundedInt)
		//
		// tokensPerDayRounded := rawTokensPerDayForRounding.Mul(round)
		// fmt.Printf("              post: %+v\n", tokensPerDayRounded)
		// fmt.Printf("              post: %+v\n", fRounded.String())
		// assert.Equal(t, expectedRoundedValue, tokensPerDayRounded.BigInt().String())

		// We use floor to ensure we are always underesimating total tokens per day
		//tokensPerDayFloored := NewBig257()
		//tokensPerDayFloored, _ = rawTokensPerDay.Int(tokensPerDayFloored)
		//
		//precision := (math.Pow(10, 15) - float64(1)) / math.Pow(10, 15)
		//
		//// Round down to 15 sigfigs for double precision, ensuring know errouneous round up or down
		//tokensPerDayDecimal := rawTokensPerDay.Mul(rawTokensPerDay, big.NewFloat(precision))
		//
		//fmt.Printf("tokensPerDayFloored: %s\n", tokensPerDayFloored.String())
		//fmt.Printf("tokensPerDayDecimal: %s\n", tokensPerDayDecimal.String())
