package _02503040918_slashingEvents
...
func (sm *StateMigration) GetMigrationName() string {
	return "02503040918_slashingEvents"
}
...
func (sm *StateMigration) MigrateState(currentBlockNumber uint64) ([][]byte, map[string][]interface{}, error) {
	...
	query := `
		...
		where
		    ...
		    and (
				(tl.address = @allocationManagerAddress and event_name in @allocationManagerEventNames)
		  		or (tl.address = @delegationManagerAddress and event_name in @delegationManagerEventNames)
		  	)
	...
	res := sm.db.Raw(query,
		...
		sql.Named("allocationManagerEventNames", []string{
			"OperatorSetCreated",
			"AllocationUpdated",
			"OperatorSlashed",
