...
func (ops *OperatorSetModel) sortValuesForMerkleTree(splits []*OperatorSet) ([]*base.MerkleTreeInput, error) {
	inputs := make([]*base.MerkleTreeInput, 0)
	for _, split := range splits {
		slotID := base.NewSlotID(split.TransactionHash, split.LogIndex)
		value, err := ops.formatMerkleLeafValue(split.BlockNumber, split.OperatorSetId, split.Avs)
		if err != nil {
