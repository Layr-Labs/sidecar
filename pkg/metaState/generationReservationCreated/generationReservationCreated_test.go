...
func Test_GenerationReservationCreated(t *testing.T) {
	...
	generationReservationCreatedModel, err := NewGenerationReservationCreatedModel(grm, l, cfg, msm)
	assert.Nil(t, err)

	t.Run("Should insert a generationReservationCreated event", func(t *testing.T) {
		block := &storage.Block{
			Number:    20535299,
			Hash:      "",
			BlockTime: time.Time{},
		...
		log := &storage.TransactionLog{
			...
			EventName:        "GenerationReservationCreated",
		...
		err := generationReservationCreatedModel.SetupStateForBlock(block.Number)
		assert.Nil(t, err)

		isInteresting := generationReservationCreatedModel.IsInterestingLog(log)
		assert.True(t, isInteresting)

		state, err := generationReservationCreatedModel.HandleTransactionLog(log)
		assert.Nil(t, err)

		typedState := state.(*types.GenerationReservationCreated)
		...
		_, err = generationReservationCreatedModel.CommitFinalState(block.Number)
		assert.Nil(t, err)

		// Check if the generationReservationCreated event was inserted
		var generationReservationCreated types.GenerationReservationCreated
		res = grm.Model(&types.GenerationReservationCreated{}).Where("block_number = ?", block.Number).First(&generationReservationCreated)
		assert.Nil(t, res.Error)

		err = generationReservationCreatedModel.CleanupProcessedStateForBlock(block.Number)
		assert.Nil(t, err)

	})

	t.Run("Should insert another generationReservationCreated event with different operator set", func(t *testing.T) {
		block := &storage.Block{
			Number:    20535362,
			Hash:      "",
			BlockTime: time.Time{},
