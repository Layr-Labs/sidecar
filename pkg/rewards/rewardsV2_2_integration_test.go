...
func Test_RewardsV2_2_WithWithdrawalQueue_Integration(t *testing.T) {
	...
	t.Run("Scenario 1: Forward-looking rewards with withdrawal during reward period", func(t *testing.T) {
		...
		logRewardsData(t, grm, snapshotDates)
	})
...
func cleanupTestData(t *testing.T, grm *gorm.DB) {
	t.Helper()
	...
	for _, table := range tables {
		...
		if result.Error != nil {
			...
			t.Logf("Cleaned table %s: %d rows deleted", table, result.RowsAffected)
		}
	}

	// Drop temporary gold tables (with date suffixes)
	var goldTables []string
	err := grm.Raw(`
		SELECT table_name
		FROM information_schema.tables
		WHERE table_name LIKE 'gold_%'
		AND table_name != 'gold_table'
	`).Scan(&goldTables).Error

	if err == nil {
		for _, table := range goldTables {
			result := grm.Exec(fmt.Sprintf("DROP TABLE IF EXISTS %s CASCADE", table))
			if result.Error != nil {
				t.Logf("Warning: Failed to drop table %s: %v", table, result.Error)
			} else {
				t.Logf("Dropped temporary table %s", table)
			}
		}
	}

	// Drop temporary operator allocation snapshot tables (with date suffixes)
	var allocationSnapshotTables []string
	err = grm.Raw(`
		SELECT table_name
		FROM information_schema.tables
		WHERE table_name LIKE 'operator_allocation_snapshots_%'
		AND table_name != 'operator_allocation_snapshots'
	`).Scan(&allocationSnapshotTables).Error

	if err == nil {
		for _, table := range allocationSnapshotTables {
			result := grm.Exec(fmt.Sprintf("DROP TABLE IF EXISTS %s CASCADE", table))
			if result.Error != nil {
				t.Logf("Warning: Failed to drop table %s: %v", table, result.Error)
			} else {
				t.Logf("Dropped temporary allocation snapshot table %s", table)
			}
		}
	}
...
// logRewardsData logs detailed reward data for debugging
func logRewardsData(t *testing.T, grm *gorm.DB, snapshotDates []string) {
	t.Helper()
	...
	for _, snapshotDate := range snapshotDates {
		...
		for _, s := range stakerShares {
			strategyDisplay := s.Strategy
			if len(s.Strategy) > 10 {
				strategyDisplay = s.Strategy[:10] + "..."
			}
			t.Logf("  %s | %s | %s", s.Staker, strategyDisplay, s.Shares)
		}

		// Log withdrawal queue shares
		var withdrawalShares []struct {
			Staker           string
			Strategy         string
			Shares           string
		...
		grm.Raw(`
			SELECT staker, strategy, shares, withdrawable_date
			FROM withdrawal_queue_share_snapshots
			WHERE snapshot = ?
			ORDER BY staker, strategy
		`, snapshotDate).Scan(&withdrawalShares)

		if len(withdrawalShares) > 0 {
			t.Logf("Withdrawal Queue Shares (%d rows):", len(withdrawalShares))
			for _, w := range withdrawalShares {
				strategyDisplay := w.Strategy
				if len(w.Strategy) > 10 {
					strategyDisplay = w.Strategy[:10] + "..."
				}
				t.Logf("  %s | %s | %s | withdrawable: %s",
					w.Staker, strategyDisplay, w.Shares, w.WithdrawableDate)
			}
		}
		...
		if len(rewards) > 0 {
			...
			for _, r := range rewards {
				tokenDisplay := r.Token
				if len(r.Token) > 10 {
					tokenDisplay = r.Token[:10] + "..."
				}
				t.Logf("  %s | %s | %s", r.Earner, tokenDisplay, r.Amount)
			}
		}
