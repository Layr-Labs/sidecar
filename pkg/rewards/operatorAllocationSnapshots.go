...
const operatorAllocationSnapshotsQuery = `
	...
	WITH ranked_allocation_records as (
		SELECT *,
			   ROW_NUMBER() OVER (PARTITION BY operator, avs, strategy, operator_set_id, cast(block_time AS DATE) ORDER BY oa.block_number DESC, log_index DESC) AS rn
	...
	ranked_max_magnitude_records as (
		SELECT
			...
			omm.block_number,
			omm.log_index,
			ROW_NUMBER() OVER (PARTITION BY omm.operator, omm.strategy, cast(b.block_time AS DATE) ORDER BY omm.block_number DESC, omm.log_index DESC) AS rn
