...
// Helper function to verify queued withdrawal adjustment
func verifyQueuedWithdrawalAdjustment(t *testing.T, grm *gorm.DB, staker, strategy, expectedMultiplier string) {
	t.Helper()
	...
	res := grm.Raw(`
		...
		LIMIT 1
	`, staker, strategy).Scan(&multiplier)
	require.NoError(t, res.Error, "Failed to query queued_withdrawal_slashing_adjustments")
	require.Equal(t, expectedMultiplier, multiplier, "Unexpected slash multiplier")
}

// Helper function to assert staker shares
func assertStakerShares(t *testing.T, grm *gorm.DB, staker, strategy, expectedShares string) {
	t.Helper()

	var shares string
	res := grm.Raw(`
		SELECT shares FROM staker_shares
		WHERE staker = ? AND strategy = ?
		ORDER BY block_number DESC, log_index DESC
		LIMIT 1
	`, staker, strategy).Scan(&shares)
	require.NoError(t, res.Error, "Failed to query staker_shares")
	require.Equal(t, expectedShares, shares, "Unexpected staker shares")
}

// Helper function to assert snapshot value
func assertSnapshot(t *testing.T, snapshots []StakerShareSnapshot, snapshotDate, expectedShares string) {
	t.Helper()

	found := false
	for _, s := range snapshots {
		if s.Snapshot.Format("2006-01-02") == snapshotDate {
			require.Equal(t, expectedShares, s.Shares, fmt.Sprintf("Unexpected shares for snapshot %s", snapshotDate))
			found = true
			break
		}
	}
